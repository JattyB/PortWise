...
# --- smb auth (fake ExternalTool) --------------------------------------------

class _FakeNxc:
    def __init__(self, stdout, *, available=True):
        self.available = available
        self._stdout = stdout

    def run(self, args, *, timeout=None, handoff_command=None, input_text=None):
        r = ExternalToolResult(tool="nxc", available=self.available, handoff_command=handoff_command)
        if self.available:
            r.ran = True
            r.returncode = 0
...
def test_run_smb_auth_pwned_critical():
    ...
    findings, notes = run_smb_auth("10.0.0.1", creds, tool=_FakeNxc("SMB  10.0.0.1  445  [+] CORP\\admin:pw (Pwn3d!)"))
...
def test_run_smb_auth_not_accepted():
    ...
    findings, notes = run_smb_auth("10.0.0.1", creds, tool=_FakeNxc("SMB  10.0.0.1  445  [-] x:y STATUS_LOGON_FAILURE"))
    assert findings == []
    assert any("not accepted" in n for n in notes)


def test_run_smb_auth_absent_binary_handoff():
    creds = [Credential(service="smb", username="x", password="y")]
    findings, notes = run_smb_auth("10.0.0.1", creds, tool=_FakeNxc("", available=False))
    assert findings == []
    assert any("handoff" in n for n in notes)
